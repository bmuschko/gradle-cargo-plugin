...
class CargoPlugin implements Plugin<Project> {
    ...
    private void setLocalJbossConventionMapping(Project project, CargoPluginConvention cargoConvention, Action action) {
        project.tasks.withType(LocalJbossTask).whenTaskAdded { LocalJbossTask localJbossTask ->
            ...
            localJbossTask.conventionMapping.map('jrmpInvokerPort') {
                CargoProjectProperty.getTypedProperty(project, LocalJbossTaskProperty.JRMP_INVOKER_PORT, cargoConvention.local.jboss.jrmpInvokerPort)
            }
